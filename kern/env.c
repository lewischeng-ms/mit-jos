...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	// warn("region_alloc: what corner-cases?");
}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
    ...
    // Must load that env's page directory for memmove to work properly.
    ...
    // Must NOT restore kernel page directory!
    // Next step is going to execute the init program!
}
...
env_create(uint8_t *binary, size_t size, enum EnvType type)
{
    ...
    if (type == ENV_TYPE_FS)
        e->env_tf.tf_eflags |= FL_IOPL_3; /* access to any IO port is granted */
    else
